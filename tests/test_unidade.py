...
class TesteUnitarioCalcula(unittest.TestCase):
    ...
    def test_tipagem_invalida_soma_boolean(self):
        """
        teste de tipagem invalida string no lugar de numero boolean
        """
        with self.assertRaises(TypeError):
            self.calc.somar(5, True)  # Boolean no lugar de numero na soma

    def test_tipagem_invalida_potencia_string(self):
        """
        teste de tipagem invalida string no lugar de numero potência
        """
        with self.assertRaises(TypeError):
            self.calc.potencia("5", 3)  # String no lugar de numero na soma

    def test_tipagem_invalida_potencia_none(self):
        """
        teste de tipagem invalida string no lugar de numero none
        """
        with self.assertRaises(TypeError):
            self.calc.potencia(5, None)  # None no lugar de numero na soma

    def test_tipagem_invalida_potencia_boolean(self):
        """
        teste de tipagem invalida string no lugar de numero boolean
        """
        with self.assertRaises(TypeError):
            self.calc.potencia(5, True)  # Boolean no lugar de numero na soma
